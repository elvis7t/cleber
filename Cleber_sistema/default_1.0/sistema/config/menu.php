...
$rs = new recordset();
$rs2 = new recordset();
$rs->Seleciona("*","menu_triangulo","mnu_niveis like '%[".$_SESSION['classe']."]%' AND mnu_hier='P'","","mnu_cod ASC");
?>
        ...
        <ul class="sidebar-menu">
            ...
            while ($rs->GeraDados()) {?>
                <li <?php if($sec == $rs->fld("mnu_sess")){ echo 'class="active"';}?>>
                    <a href="<?= $hosted."/view/".$rs->fld("mnu_link").$token; ?>&cnpj=<?=$_SESSION['usu_empresa'];?>" class="<?=($_SESSION[$rs->fld("mnu_sysSessao")]==0?"hide":"");?>">
                    ...
                    <ul class="treeview-menu">
						<?php
						$rs2->Seleciona("*","menu_triangulo","mnu_sess ='".$rs->fld("mnu_sess")."' AND mnu_cod > ".$rs->fld("mnu_cod")." AND mnu_niveis like '%[".$_SESSION['classe']."]%' AND mnu_hier='F'");
						echo "<!-- {$rs2->sql}-->";
						while ($rs2->GeraDados()):?>
							<li class='<?=($rs2->fld("mnu_link") == $pag ? "active" : "");?>'>
								<a href="<?= $hosted."/view/".$rs2->fld("mnu_link").$token; ?>&cnpj=<?=$_SESSION['usu_empresa'];?>" class="<?=($_SESSION[$rs->fld("mnu_sysSessao")]==0?"hide":"");?>">
