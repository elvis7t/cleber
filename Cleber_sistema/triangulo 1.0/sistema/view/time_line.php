...
$arr = array();
$datas->Seleciona("tem_data","ltempo","tem_id > 0",""," tem_hora DESC");
...
if(sizeof($arr)> 0) {
	for($i=0; $i<sizeof($arr); $i++){
		$sql = "SELECT * FROM ltempo
				JOIN usuarios ON tem_id = usu_cod
				WHERE tem_data = '".$arr[$i]."' ORDER BY tem_hora DESC LIMIT 8";
		$dash->FreeSql($sql);
		if($dash->linhas > 0){
			$dash->GeraDados();?>
				<ul class="timeline">
		    			...
		    			while ($dash->GeraDados()){?>
		    				<li>
				        		...
				        		<div class="timeline-item">
				            		<span class="time"><i class="fa fa-clock-o"></i> <?=date("H:i:s",$dash->fld("tem_hora"));?></span>
