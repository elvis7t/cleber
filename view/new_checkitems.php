					...
					<div class="box box-primary">
						...
						<form role="form" id="cad_cham">
							...
							<div class="box-body">
								...
								<div id="clientes" class="row">
									...
									<div class="form-group col-md-5">
										...
										<select class="select2" style="width:100%" name="sel_maq" id="sel_maq">
											<option value="">Selecione:</option>
											<?php
											
											// Verifica quais titulos existem na tabela Obrigações para a empresa
											$impos = array();
											$sql = "SELECT lver_maquina FROM listados WHERE lver_listaId = ".$_GET['lista'];
											
											$rs->FreeSql($sql);
											while($rs->GeraDados()){
												$impos[] = $rs->fld("lver_maquina");
											}

											$whr = " AND maq_ativa = 1 AND maq_empvinc=".$_SESSION['sys_id']." AND maq_cliente = ".$_GET['emp'] ;
										...
										?>	
											<div class="form-group col-md-3">
												...
												if($rs->fld("task_campo")=="VAL"){ ?>
													<input title="Campo Obrigatório" reqired type="text" class="form-control quest" name="<?=$rs->fld("task_id")?>" id="<?=$rs->fld("task_id")?>" />
												<?php
												}
												else{?>
													<select title="Campo Obrigatório" required class="select2 quest" name="<?=$rs->fld("task_id");?>" id="<?=$rs->fld("task_id");?>" style="width:100%;">
														<option value="">Selecione:</option>
								...
								<div class="row">
									<div class="form-group col-md-12 pull-right">
										...
										<textarea class="form-control" id="check_obs" name="check_obs">
										</textarea>
							...
							<div class="box-footer">
								<button class="btn btn-sm btn-success" type="button" id="bt_add_check"><i class="fa fa-laptop"></i> Incluir</button>
								<a class="btn btn-sm btn-danger" href="javascript:history.go(-1);"><i class="fa fa-times"></i> Cancelar</a>
							</div>
						</form>
					</div><!-- ./box -->
					</div>
