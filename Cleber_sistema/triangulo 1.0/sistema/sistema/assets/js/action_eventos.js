	...
	$("#btn-partok").on("click", function(){
		...
		$("#participe").validate({
            ...
            messages: {
                ...
                eve_data1: {required: "Escolha uma data para o primeiro pagamento"},
                eve_data2: {required: "Escolha uma data para o segundo pagamento"}
            }
        });
			if($("#participe").valid()==true){
				$("#participar").modal("hide");
				$.post("../controller/PRIEventos.php",{
					acao		: "Participar",
					evento		: $("#evento").val(),
					cnpj		: $("#cnpj").val(),
					valor		: $("#valor").val(),
					valor_ext	: $("#valorpext").val(),
					venc1		: $("#eve_data1").val(),
					venc2		: $("#eve_data2").val(),
					forma		: $("#rd_pag:checked").val(),
					manual		: $("#chkrecebe:checked").val()
					},
					function(data){
						if(data.st=="OK"){
							$("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> Participante cadastrado com sucesso na PRIORE! <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
							location.reload();
						} else{
							$("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
						}
...
/*------------------------------------ no caso de enviar para outro CNPJ - SELECT ----------------------------*/
	$("#man_cnpj").on("change",function(){
		$("#cnpj").val($("#man_cnpj").val());
		console.log($("#man_cnpj").val());
	});
/*------------------------------------ no caso de alterar o valor do participante - INPUT ----------------------------*/
	$("#man_valor").on("change", function(){
		var ini = $("#valor").val();
		if($(this).val()==""){$("#valor").val(ini);}
		else{$("#valor").val($(this).val());}
	});
