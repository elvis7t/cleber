$(document).on("ready", function(){
	...
	\*-------------------------------------------------------------*/
		$(document.body).on("click",".irpflink",function(e){
			...
			$("#ir_ocorr").validate({
	            ...
	            wrapper: 'li',
	            rules: {
	                ir_obs: {required: true},
	                ir_acao	:{required:true}
	            },
	            messages: {
	                ir_obs: {required: "Registro de ocorr&ecirc;ncia Obrigat&oacute;rio!"},
	...
	/*|DESCONTO DE HORAS|*/
		$(document.body).on("keyup","#dhor_desc",function(){
			var n1, n2, disp;
			n1 = parseFloat($(this).val());
			n2 = parseFloat($("#dhor_disp").val());
			disp = n2 - n1;
			
			if(n1 > n2){
				alert("Não é possível descontar mais horas que o disponível");
				$("#dhor_disp").val(n2.toFixed(2));
				$("#dhor_desc").val("");
				$("#dhor_saldo").val("");
			}
			else{
				$("#dhor_saldo").val(disp.toFixed(2));
			}
		});
	/*|INCLUINDO DESCONTO|*/
