...
$(document.body).on("click","#bt_altcli", function(){
    ...
    if ($("#cad_clientes").valid() == true) {
        ...
        $.post("../controller/TRIEmpresas.php", {
            ...
            cli_mail:   $("input[name=cli_mail]:checked").val(),
            cli_malote:     $("input[name=cli_malote]:checked").val(),
            cli_tribut:     $("#cli_tribut").val(),
            cli_dataim:     $("#cli_dataim").val(),
            cli_datatrib:   $("#cli_datatrib").val(),
            cli_datacnpj:   $("#cli_datacnpj").val(),
            cli_usaca:      $("#emp_ca").val(),
            cli_capor:      $("#emp_capor").val(),
...
/*---------------|Adicionar mensagem ao aguardar|---------------*\
| Author:   Cleber Marrara Prado                                |
| Version:  1.0                                                 |
| Email:    cleber.marrara.prado@gmail.com                      |
| Date:     13/11/2017                                          |
\*-------------------------------------------------------------*/
$(document.body).on("click","#link_acessar", function(){
...
$(document.body).on("click","#bt_cadObr",function(){
    ...
    $("#cad_obr").validate({
        ...
        rules: {
            ...
            ob_venc:    {required: true},
            ob_ativo:   {required: true}
        },
        messages: {
            ob_titulo:  {required: "Escolha a obrigação"},
            ob_venc:    {required: "Digite um dia para o vencimento:"},
...
/*----------|CADASTRO DE CERTIDÕES DO CLIENTE|-----------------*\
| Author:   Cleber Marrara Prado                                |
| Version:  1.0                                                 |
| Email:    cleber.marrara.prado.com                            |
| Date:     23/05/2018                                          |
| Objetivo: Cadastrar certidções lembrete de validades          |
\*-------------------------------------------------------------*/
$(document.body).on("click","#bt_cadCertid",function(){
    tipo = "cadastrar";
    ...
    $("#cad_cert").validate({
        ...
        wrapper: 'li',
        rules: {
            certid_tipo     : {required: true},
            certid_val      : {required: true},
            certid_status   : {required: true}
        },
        messages: {
            certid_tipo     :  {required: "Escolha a certidão"},
            certid_val      :  {required: "Digite um dia para o vencimento:"},
            certid_status   :  {required: "Escolha uma opção para a certidão"}
        },
        highlight: function(element) {
            $(element).closest('.form-group').addClass('has-error');
        },
    ...
    if ($("#cad_cert").valid() == true) {
        $.post("../controller/TRIEmpresas.php", {
            acao:          $("#acao").val(),
            certid_id:     $("#certid_id").val(), 
            certid_cod:    $("#certid_cod").val(), 
            certid_tipo:   $("#certid_tipo").val(),
            certid_val:    $("#certid_val").val(),
            certid_status: $("#certid_status").val()
        },
                function (data) {
                    if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> ('+data.mensagem+') <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                        console.log(data.sql);//TO DO mensagem OK
                        //limpa_formulario_all();
                        location.href = "clientes.php?token=" +$("#token").val()+"&cnpj="+$("#cnpj").val()+"&clicod="+$("#certid_cod").val()+"&tab=certid";
                    }
                    else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-flag-checkered"></i> ('+data.mensagem+') <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                    }
...
$(document.body).on("click",".exc_Certids",function(){
    console.log("CLICK OK");
    cod = $("#cod").val();
    $.post("../controller/TRIEmpresas.php",{
        acao: "exclui_certid",
        cert_id: $(this).data("reg"),
        cod : cod    
    },
    function(data){
        if(data.status=="OK"){
...
$(document.body).on("change","#cf_colab",function(){
    ...
    if($(this).val()==0){
        $("#gcart2").attr("disabled",true);
    }
    else{
        $("#gcart2").attr("disabled",false);
        
    }
    */
});

$(document.body).on("click","#gcart2", function () {
    ...
    if(checkeditems==""){
        ...
        $.post("../controller/TRIEmpresas.php",
            ...
            function(data){
                if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_f");
                        $("#fisPesq").trigger("click");
                        
                    } else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! ('+data.mensagem+')<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_f");
                    }
                    console.log(data.sql);//TO DO mensagem OK
            },
            "json"
    ...
    $("#gcart2").html("<i class='fa fa-user'></i> Associar");
});     
...
$(document.body).on("change","#cc_colab",function(){
    ...
    console.log($(this).val());
    if($(this).val()==0){
        $("#gcart1").attr("disabled",true);
    }
    else{
        $("#gcart1").attr("disabled",false);
        
    }
    */
});

$(document.body).on("click","#gcart1", function () {
    $(this).html("<i class='fa fa-spinner fa-spin'></i> Gerando...");
    ...
    if(checkeditems==""){
        ...
        $.post("../controller/TRIEmpresas.php",
            ...
            function(data){
                if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_c");
                        $("#ctPesq").trigger("click");
                        
                    } else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! ('+data.mensagem+')<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_c");
                    }
                    console.log(data.sql);//TO DO mensagem OK
            },
            "json"
    ...
    $("#gcart1").html("<i class='fa fa-user'></i> Associar");
});     
...
$(document.body).on("click","#ctPesq",function(){
    ...
    var level = $("#pes_ctlevel").val();
    //var por = $("#pes_dpPorte").val();
    console.log("vis_cartcontabil.php?emp="+emp+"&tip="+tip+"&level="+level/*+"&por="+por*/);
    $("#respct").load("vis_cartcontabil.php?emp="+emp+"&tip="+tip+"&level="+level/*+"&por="+por*/);
    $("#bt_cartcon_rel").attr("href","../rel/rel_cartcontabil.php?emp="+emp+"&tip="+tip+"&level="+level+"&r=1");
    $("#bt_cartcon_excel").attr("href","../rel/rel_cartcontabil_excel.php?emp="+emp+"&tip="+tip+"&level="+level+"&r=1");
});
...
$(document.body).on("click","#fisPesq",function(){
    ...
    console.log("../rel/rel_cartfiscal_excel.php?emp="+emp+"&tri="+tri+"&imp="+imp+"&tip="+tip+"&r=1");
    ...
    $("#bt_cartfis_excel").attr("href","../rel/rel_cartfiscal_excel.php?emp="+emp+"&tri="+tri+"&imp="+imp+"&tip="+tip+"&r=1");
});
...
$(document.body).on("click","#dpPesq",function(){
    ...
    $("#bt_cartdp_rel").attr("href","../rel/rel_cartdp.php?emp="+emp+"&tip="+tip+"&por="+por+"&r=1");
    $("#bt_cartdp_excel").attr("href","../rel/rel_cartdp_excel.php?emp="+emp+"&tip="+tip+"&por="+por+"&r=1");
});

/*---------------|FILTRO CARTEIRA LEGAL|-----------------------*\
| Author:   Cleber Marrara Prado                                |
| Version:  1.0                                                 |
| Email:    cleber.marrara.prado@gmail.com                      |
| Date:     30/09/2016                                          |
\*-------------------------------------------------------------*/
$(document.body).on("click","#legPesq",function(){
    tipo = "aguarde";
    var emp = ($("#pes_legEmpresa").val()===null?"":$("#pes_legEmpresa").val());
    var tip = $("#pes_legTipo").val();
    console.log("vis_cartlegal.php?emp="+emp+"&tip="+tip);
    $("#respleg").load("vis_cartlegal.php?emp="+emp+"&tip="+tip);
    $("#bt_cartleg_rel").attr("href","../rel/rel_cartleg.php?emp="+emp+"&tip="+tip+"&r=1");
    $("#bt_cartleg_excel").attr("href","../rel/rel_cartleg_excel.php?emp="+emp+"&tip="+tip+"&r=1");
});

/*---------------|FILTRO CARTEIRA FINANCEIRO|------------------*\
| Author:   Cleber Marrara Prado                                |
| Version:  1.0                                                 |
| Email:    cleber.marrara.prado@gmail.com                      |
| Date:     30/09/2016                                          |
\*-------------------------------------------------------------*/
$(document.body).on("click","#finPesq",function(){
    tipo = "aguarde";
    var emp = ($("#pes_finEmpresa").val()===null?"":$("#pes_finEmpresa").val());
    var tip = $("#pes_finTipo").val();
    console.log("vis_cartfinan.php?emp="+emp+"&tip="+tip);
    $("#respfin").load("vis_cartfinan.php?emp="+emp+"&tip="+tip);
    $("#bt_cartfin_rel").attr("href","../rel/rel_cartfin.php?emp="+emp+"&tip="+tip+"&r=1");
    $("#bt_cartfin_excel").attr("href","../rel/rel_cartfin_excel.php?emp="+emp+"&tip="+tip+"&r=1");
});
...
$(document.body).on("change","#cd_colab",function(){
    ...
    console.log($(this).val());
    if($(this).val()==0){
        $("#gcart4").attr("disabled",true);
    }
    else{
        $("#gcart4").attr("disabled",false);
        
    }
    */
});

$(document.body).on("click","#gcart4", function () {
    $(this).html("<i class='fa fa-spinner fa-spin'></i> Gerando...");
    ...
    if(checkeditems==""){
        ...
        $.post("../controller/TRIEmpresas.php",
            ...
            function(data){
                if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_d");
                        $("#dpPesq").trigger("click");
                        
                    } else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! ('+data.mensagem+')<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_d");
                    }
                    console.log(data.sql);//TO DO mensagem OK
            },
            "json"
    ...
    $("#gcart4").html("<i class='fa fa-user'></i> Associar");
});
...
$(document.body).on("click","#gcart5", function () {
    ...
    if(checkeditems==""){
        ...
        $.post("../controller/TRIEmpresas.php",
            ...
            function(data){
                if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_l");
                        $("#legPesq").trigger("click");
                        
                    } else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! ('+data.mensagem+')<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_l");
                    }
                    console.log(data.sql);//TO DO mensagem OK
            },
            "json"
    ...
    $(document.body).on("click","#btn_pesqmensal",function(){
        ...
        $("#tbl_mensal").load("vis_mensalprod.php?empre="+emp+"&impos="+imp+"&usua="+usu+"&compet="+comp, 
            function(){
                ...
                $("#bt_mensal_excel").attr("href","../rel/rel_mensal.php?empre="+emp+"&impos="+imp+"&usua="+usu+"&compet="+comp);    
            }
        ).fadeIn("slow");
    });
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click","#btn_mosfila",function(){
        $(this).html("<i class='fa fa-spin fa-spinner'></i>");
        ...
        $.post("../controller/TRIProdutividade.php",
            ...
            function(data){
                $("#btn_mosfila").html("<i class='fa fa-search'></i>");
                $("#tbl_fila").fadeOut("slow").html(data).fadeIn("slow");
                $(".chk_imp").bootstrapToggle({
                    on: "Sim",
                    off: "Não"
        ...
        if($("#cad_filaimporta").valid()==true){
            $.post("../controller/TRIProdutividade.php",
                ...
                function(data){
                    if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                        location.href = "disp_arqs.php?token="+token+"&cnpj="+cnpj;
    ...
    $(document.body).on("click","#bt_recalc", function(){
        ...
        $("#cad_Calc").validate({
            ...
            messages: {
                ...
                calc_valor: {required:"Campo valor obrigatório"}
            },
            highlight: function(element) {
                $(element).closest('.form-group').addClass('has-error');
            },
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click",".exc_recalc",function(){
        console.log($("#pes_dataini").val());
        var di = $("#pes_dataini").val();
        var df = $("#pes_datafim").val();
        cod = $(this).data("reg");
        $.post("../controller/TRIEmpresas.php",{
            acao: "canc_calc",
            rec_id: cod
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click",".fat_recalc",function(){
        ...
        var di = $("#pes_dataini").val();
        var df = $("#pes_datafim").val();
        var sta = $("#pes_status").val();
        var cod = $(this).data("reg");
        $.post("../controller/TRIEmpresas.php",{
            acao: "fat_recalc",
            rec_id: cod
        },
        function(data){
            if(data.status=="OK"){
                alert(data.mensagem);
                console.log("recalc.php?token="+token+"&cnpj="+cnpj+"&di="+di+"&df="+df+"&status="+sta);
                location.href= "recalc.php?token="+token+"&cnpj="+cnpj+"&di="+di+"&df="+df+"&status="+sta;
            } 
            else{
                alert(data.mensagem);   
            }
...
| Date:     05/12/2016                                          |
\*-------------------------------------------------------------*/
    $(document.body).on("click","#btn_pesqrec",function(){
        var emp = $("#pes_calcemp").val();
        var col = $("#pes_colab").val();
        var sta = $("#pes_status").val();
        var di = $("#pes_dataini").val();
        var df = $("#pes_datafim").val();
        console.log("vis_recalc.php?emp="+emp+"&col="+col+"&di="+di+"&df="+df+"&status="+sta);
        $("#vis_recalc").fadeOut("fast").load("vis_recalc.php?emp="+emp+"&col="+col+"&di="+di+"&df="+df+"&status="+sta).fadeIn("slow");
        $("#rec_excel").attr("href","../rel/rel_recalcexcel.php?emp="+emp+"&col="+col+"&di="+di+"&df="+df+"&status="+sta);
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click","#bt_impla", function(){
        var container = $("#formerros_impla");
        var checkeditems = $('input:checkbox[name="imp_chk[]"]:checked')
                   .map(function() { return $(this).val() })
                   .get()
                   .join(",");
        if(checkeditems==""){
            alert("Escolha os documentos obrigatórios para a implantação!");
            ...
            $("#cad_impla").validate({
                ...
                wrapper: 'li',
                rules: {
                    imp_emp:    {required:true},
                    impl_data:   {required:true}
                },
                messages: {
                    imp_emp:    {required:"Escolha a empresa..."},
                    impl_data:   {required:"Digite a data"}
                },
                highlight: function(element) {
                    $(element).closest('.form-group').addClass('has-error');
                },
            ...
            if($("#cad_impla").valid()==true){
                $("#bt_impla").html("<i class='fa fa-cog fa-spin'></i> Processando...");
                $.post("../controller/TRIEmpresas.php",{
                    acao:       "inclui_implantacao",
                    imp_emp:    $("#imp_emp").val(),
                    imp_data:   $("#impl_data").val(),
                    imp_valor:  $("#impl_valor").val(),
                    imp_chk:    checkeditems,
                    imp_obs:    $('#impl_obs').val()
                    },
                    function(data){
                        if(data.status=="OK"){
                            $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                            $("#bt_impla").html("<i class='fa fa-hand-handshake-o'></i> Iniciar");
                            $("#cad_imp")[0].reset();
                            $("#imp_emp").select2("val","");
                            location.reload();
...
| Objetivo: Salvar homologações                                 |
\*-------------------------------------------------------------*/
    $(document.body).on("click","#bt_save_impla", function(){
        $("#bt_save_impla").html("<i class='fa fa-cog fa-spin'></i> Processando...");
            $.post("../controller/TRIEmpresas.php",{
                acao        : "salva_impla",
                impla_id      : $("#impla_id").val(),
                impla_obs : CKEDITOR.instances.editor1.getData()
                },
                function(data){
                    if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                        $("#cad_atendimp")[0].reset();
                        $("#bt_save_cham").html("<i class='fa fa-save'></i> Salvar");
                        //$("#slc").load("meus_chamados.php").fadeIn("slow");
                        location.reload();
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click",".excDocImp", function(){
        console.log("CLICK OK");
        cod = $(this).data("reg");
        $.post("../controller/TRIEmpresas.php",{
            acao: "exclui_docimpla",
            doc_id: cod
        },
        function(data){
            if(data.status=="OK"){
...
/*-----------------|ENCERRAR DE IMPLANTAÇÔES|------------------*\
| Author:   Cleber Marrara Prado                                |
| Version:  1.0                                                 |
| Email:    cleber.marrara.prado.com                            |
| Date:     12/09/2017                                          |
| Objetivo: Encerrar implantações                               |
\*-------------------------------------------------------------*/
    $(document.body).on("click","#bt_encerra_impla",function(){
        var token = $("#token").val();
        var cnpj = $("#cnpj").val();
        $.post("../controller/TRIEmpresas.php",{
            acao        : "encerra_impla",
            ...
            function(data){
                if(data.status=="OK"){
                    $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta");
                    $("#bt_encerra_impla").html("<i class='fa fa-exclamation'></i> Encerrar");
                    //$("#slc").load("meus_chamados.php").fadeIn("slow");
                    location.href = "pes_implanta.php?token="+token+"&cnpj="+cnpj;
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click",".excImpla", function(){
        console.log("CLICK OK");
        cod = $(this).data("reg");
        $.post("../controller/TRIEmpresas.php",{
            acao: "exclui_impla",
...
\*-------------------------------------------------------------*/
    ...
    $(document.body).on("click",".chkImpla", function(){
        console.log("CLICK OK");
        cod = $(this).data("reg");
        $.post("../controller/TRIEmpresas.php",{
            acao: "ok_impla",
