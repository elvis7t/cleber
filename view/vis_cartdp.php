	...
	if($rs->linhas==0):
		...
		while($rs->GeraDados()){
			...
			<tr>
				...
				<td class="<?=(isset($_GET['r'])?"hide":"")?>"><?=$rs2->pegar("tipemp_desc","tipos_empresas","tipemp_cod=".$rs->fld("tipo_emp"));?></td>
				<td class="<?=(isset($_GET['r'])?"hide":"")?>"><?=$rs2->pegar("port_func","porte_empresa","port_id=".$rs->fld("num_emp"));?></td>
				<?php
