...
if($acao == "fila_arquivo"){
	...
	$sql = "SELECT
		...
		$todos_dep= $per->getPermissao("todos_depart",$_SESSION['usu_cod']);
		...
		$sql.=" AND c.cliarq_ativo=1";
		if($emp<>""){
			$sql.=" AND (e.cod = ".$emp.")";
			$sql.=" GROUP BY tarq_id";
		}
		else{
			$sql.=" AND trata_id IS NOT NULL";
			$sql.=" GROUP BY CodO, tarq_id";
		}
	$sql.=" ORDER BY d.trata_mov DESC, d.trata_status DESC, c.cliarq_venc ASC, d.trata_erro ASC, d.trata_movdata ASC";
	$tbl = "";
	//echo $sql."<br>";
	$rs_eve->FreeSql($sql);
	...
	if($rs_eve->linhas>0){
		...
		while($rs_eve->GeraDados()){
			...
			//echo $rs_eve->fld("tarq_id")." - ".$emp_no ." - ".$emp_im;
			...
			if($emp_no<>""){
				...
				//$sql1.=" AND trata_movdata IS NOT NULL";
			}
			...
			//if($rs1->linhas>0){
				...
				$rs1->GeraDados();
				$color='';
				if($rs1->fld("st_codstatus")==-1) {
					$color = "bg-danger";
				}
				if($rs1->fld("st_codstatus")==-2) {
					$color = "bg-success";
				}
				...
				$tbl .= "<input type='hidden' name='empresa".$rs_eve->fld("tarq_id")."' id='empresa".$rs_eve->fld("tarq_id")."' value='{$emp_im}'>
						 ...
						 <tr>
						 	<td class=".$color.">".$rs1->fld("trata_id")."</td>
						 	<td class=".$color.">".$emp_im." - ". $emp_no."</td>
							<td class=".$color.">".$rs_eve->fld("tarq_nome")."</td>
							<td class=".$color.">".$mes_cp."</td>
							<td class=".$color.">".$vn."</td>
							<td class=".$color.">".($colab[$rs_eve->fld("tarq_depart")]["user"]==0?"-":$rs2->pegar("usu_nome","usuarios","usu_cod=".$colab[$rs_eve->fld("tarq_depart")]["user"]))."</td>
							<td class=".$color.">".($rs1->fld("trata_resp")==0?"-":$rs2->pegar("usu_nome","usuarios","usu_cod=".$rs1->fld("trata_resp")))."</td>
							<td class=".$color.">
							...
							<td class=".$color.">".$rs1->fld("st_desc")."</td>
							<td class=".$color.">";
...
$con = $per->getPermissao("ver_empresas", $_SESSION['usu_cod']);
	$sql = "SELECT 
				...
				JOIN clientes_arquivos b 	ON b.cliarq_arqId = a.trata_cliarqarqid AND b.cliarq_empresa=a.trata_cliarqEmp
				JOIN tri_clientes c 		ON a.trata_cliarqEmp=c.cod
				JOIN tipos_arquivos d 		ON a.trata_cliarqarqid=d.tarq_id
				JOIN codstatus e			ON e.st_codstatus=a.trata_status
				WHERE 
					1
				";
		...
		if($con['C']==0){
			$sql.= " AND c.carteira LIKE '%\"user\":\"".$_SESSION['usu_cod']."\"%'";
		}
		...
		if(isset($comp) AND $comp<>""){
			...
			$mes_cp = ( date('d') > 26 ? date("m/Y", strtotime("first day of this month")) : date("m/Y", strtotime("-1 month")) );
			$mes_vn = ( date('d') > 26 ? date("m/Y", strtotime("first day of next month")) : date("m/Y") );
			$sql.=" AND trata_competencia = '".$mes_cp."'";
		}
	...
	if($rs_eve->linhas>0){
		...
		while($rs_eve->GeraDados()){
			...
			$color='';
			if($rs_eve->fld("st_codstatus")==-1) {
				$color = "bg-danger";
			}
			if($rs_eve->fld("st_codstatus")==-2) {
				$color = "bg-success";
			}	
				...
				$fura = $per->getPermissao("fura_fila",$_SESSION['usu_cod']);
				$mmov = $menv = '';
				if($rs_eve->fld("trata_mov")==1){$mmov.="Movimento gerado por: em: <br>";}
				if($rs_eve->fld("trata_status")==99){
					$menv.="Enviado: ".$rs2->pegar("usu_nome","usuarios","usu_cod=".$rs_eve->fld("trata_finpor"))." em ".$fn->data_hbr($rs_eve->fld("trata_stdata"));
				}
				$tbl .= "<input type='hidden' name='empresa".$rs_eve->fld("tarq_id")."' id='empresa".$rs_eve->fld("tarq_id")."' value='{$emp_im}'>
						 <input type='hidden' name='compet' id='compet' value='".$mes_cp."'>
						 <tr>
						 	<td class=".$color.">".$rs_eve->fld("trata_id")."</td>
						 	<td class=".$color.">".$emp_im." - ". $emp_no."</td>
							<td class=".$color.">".$rs_eve->fld("tarq_nome")."</td>
							<td class=".$color.">".$mes_cp."</td>
							<td class=".$color.">".$vn."</td>
							<td class=".$color.">".($colab[$rs_eve->fld("tarq_depart")]["user"]==0?"-":$rs2->pegar("usu_nome","usuarios","usu_cod=".$colab[$rs_eve->fld("tarq_depart")]["user"]))."</td>
							<td class=".$color.">".($rs_eve->fld("trata_resp")==0?"-":$rs2->pegar("usu_nome","usuarios","usu_cod=".$rs_eve->fld("trata_resp")))."</td>
							
							<td class=".$color.">".$rs_eve->fld("st_desc")."</td>
							<td class=".$color.">";
					if($rs_eve->fld("trata_mov")==1 && $rs_eve->fld("trata_status")==92){
						$tbl.="
								<a 
									class='btn btn-xs btn-info pesq '".(($i>1 AND $fura['C']==0)?'disabled':'active')."
									title='Iniciar processo de importação'
									href=".(($i>1 AND $fura['C']==0)?"javascript:alert('Indisponivel');":"tratamento_arquivos.php?arq=".$rs_eve->fld("trata_id")."&token=".$_SESSION["token"])."
									role='button'>
									<i class='fa fa-check-square-o'></i> 
								</a>";
					}
...
if($acao == "fin_arquivo"){
	...
	$dados['trata_stdata'] 	= date("Y-m-d H:i:s");
	$dados['trata_finpor']  = $_SESSION['usu_cod'];
	$whr = "trata_id=".$arq;
	if(!$rs_eve->Altera($dados,"trata_arquivos",$whr)){
		$result['status'] = "OK";
