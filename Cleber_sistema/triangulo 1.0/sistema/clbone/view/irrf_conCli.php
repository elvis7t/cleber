<?php
	...
	*/
			...
			<?php if($rel == 0): ?>
				...
				<?php if($rs->fld("ir_status")<>90):?>
					<a 	href="irpf_ocorrencia.php?token=<?=$_SESSION['token']; ?>&clicod=<?=$_GET['clicod']; ?>&ircod=<?=$rs->fld("ir_Id"); ?>"
						...
						<i class='fa fa-tags'></i> Novo Evento 
					</a>
					...
					<?php
						if($rs->fld("st_codstatus") == 89): ?>
							<a 	href="irpf_receita.php?token=<?=$_SESSION['token']; ?>&clicod=<?=$_GET['clicod']; ?>&ircod=<?=$rs->fld("ir_Id"); ?>"
								...
								<i class='fa fa-eye'></i> Status Receita
							</a>
						<?php
						else: ?>
							<a 	href="irpf_alteracao.php?token=<?=$_SESSION['token']; ?>&clicod=<?=$_GET['clicod']; ?>&ircod=<?=$rs->fld("ir_Id"); ?>"
								...
								data-placement='bottom' 
								title='Alterar IRPF'>
								<i class='fa fa-terminal'></i> Alterar
							</a>
						<?php
						endif;
					else: ?>
						...
						data-toggle='tooltip' 
						data-placement='bottom'>
						<i class='fa fa-thumbs-o-down'></i> Cancelado
					</a>
					<?php 
					endif;	
					?>
	...
	<?php endif; ?>
		<tr>
			<td colspan=8>
				<?php if(isset($pag) AND $pag=="dados_emp.php"):?>
				<a href="novo_irrf.php?token=<?=$_SESSION['token'];?>&clicod=<?=$_GET['clicod']; ?>"
					class='btn btn-sm btn-success' 
					data-toggle='tooltip' 
					data-placement='bottom' 
					title='Novo IRPF'><i class='fa fa-pie-chart'></i> Novo IRPF
				</a>
				<?php endif; 
				?>
			</td>
