...
define('DB_HOSTNAME','priore-sys.mysql.uhserver.com');
define('DB_USERNAME','br_admin');
define('DB_PASSWORD','priore*12');
define('DB_DATABASE','priore_sys');
...
$sql = "SELECT * FROM empresas a
		JOIN tab_municipios b
			ON a.emp_cidade = b.nome
		JOIN contatos c
			ON a.emp_cnpj = c.con_cli_cnpj
		WHERE emp_cnpj = '23.072.748/0001-03' ORDER BY c.con_contato ASC";
$result = mysqli_query($conn, $sql) or die(mysqli_error($conn));
$regs = mysqli_fetch_assoc($result);
// --------------------------------------------------------------------------------------------------------------------------

// Criar Chave da NFe -------------------------------------------------------------------------------------------------------	
$uf		= $regs['iduf']; // Código da UF - 2 dígitos
$aamm 	= date("ym"); // Ano e Mes da emissão - 4 dígitos
$cnpj 	= $fn->sanitize($regs['emp_cnpj']); // CNPJ com 14 dígitos - só números
...
$nfe = new MakeNFe();
// dados da tag infNFe -------------------------------------------------------------------------------------------------------
$vers = "3.10";  //Versão da NFe
$resp = $nfe->taginfNFe($ch, $vers);
// ---------------------------------------------------------------------------------------------------------------------------
// dados da tag <ide> -------------------------------------------------------------------------------------------------------- 
$cUF 	= $uf; //codigo numerico do estado
$cNF 	= $codNf; //numero aleatório da NF
...
$dhEmi 	= date("d/m/Y");  //para versão 3.00 '2014-02-03T13:22:42-3.00' não informar para NFCe
...
$cDV 	= $prixml->dv; //digito verificador
...
$verProc = '1.0.0'; //versão do aplicativo emissor
...
// ---------------------------------------------------------------------------------------------------------------------------
// dados da tag emit ---------------------------------------------------------------------------------------------------------
$CNPJ 	= $fn->sanitize($regs['emp_cnpj']);
$CPF 	= '';
$xNome 	= $regs['emp_razao'];
$xFant 	= $regs['emp_nome'];
$IE 	= $fn->sanitize($regs['emp_ie']);
$IEST 	= '';
$IM 	= $fn->sanitize($regs['emp_im']);
$CNAE 	= $fn->sanitize($regs['emp_cnae']);
$CRT 	= $fn->sanitize($regs['emp_crt']);
...
$nro 	= $regs['emp_num'];
$xCpl 	= $regs['emp_compl'];
$xBairro = $regs['emp_bairro'];
$cMun 	= $regs['id'];
$xMun 	= $regs['emp_cidade'];
$UF 	= $regs['emp_uf'];
$CEP 	= $regs['emp_cep'];
$cPais 	= '1058';
$xPais 	= 'BRASIL';
$fone 	= $fn->sanitize($regs['con_contato']);
...
$sql = "SELECT * FROM empresas a
		...
		LEFT JOIN contatos c
			ON a.emp_cnpj = c.con_cli_cnpj
		WHERE emp_cnpj = '11.871.919/0001-77' ORDER BY c.con_contato ASC";
$result = mysqli_query($conn, $sql) or die(mysqli_error($conn));
$regs = mysqli_fetch_assoc($result);
$cnpj 	= str_pad($fn->sanitize($regs['emp_cnpj']),14,"0",STR_PAD_LEFT);
$cpf 	= '';
$idEstrangeiro = '0';
$xNome 	= $regs['emp_razao'];
$indIEDest ='0';
$numIE 	= $fn->sanitize($regs['emp_ie']);
$isUF 	= '';
$numIM 	= '';
$email 	= '';
$resp = $nfe->tagdest($cnpj, $cpf, $idEstrangeiro, $xNome, $indIEDest, $numIE, $isUF, $numIM, $email);
// ---------------------------------------------------------------------------------------------------------------------------
