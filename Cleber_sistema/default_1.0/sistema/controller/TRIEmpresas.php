...
if($acao == "incluir_cli"){
	...
	$dados['site'] 			= $cli_site;
	$dados['telefone'] 		= $cli_tel;
	$dados['emp_vinculo'] 	= $_SESSION['usu_empcod'];
	$dados['obs'] 			= $cli_obs;
...
if($acao == "imp_pesq"){
	...
	$sql.=" AND c.ob_ativo=1 ORDER BY a.imp_id ASC";//, cast(a.imp_venc as unsigned integer) ASC";
	$tbl = "";
	//echo $sql."<br>";
	$rs_eve->FreeSql($sql);
	if($rs_eve->linhas>0){
		while($rs_eve->GeraDados()){
			...
			//if($rs1->linhas>0){
				...
				if($rs_eve->fld("imp_regra")=="mes_subs"){
					...
					$vaj = $rs_eve->fld("imp_venc"); 
					$vn = (($rs_eve->fld("imp_venc")<>"" AND $rs_eve->fld("imp_venc")<>0)?$fn->data_br($fn->dia_util($vaj,"dia_util",$ref2)):$fn->ultimo_dia_mes($ref));
				}
				else{
					
					$vn =  $fn->data_br($fn->dia_util($rs_eve->fld("imp_venc"),$rs_eve->fld("imp_regra"),$cc));
				...
				$tbl .= "<input type='hidden' name='empresa".$rs_eve->fld("imp_id")."' id='empresa".$rs_eve->fld("imp_id")."' value='{$emp_im}'>
						 ...
						 <tr>
							...
							<td>".$vn."</td>
							<td>".($colab[$rs_eve->fld("imp_depto")]["user"]==0?"-":$rs2->pegar("usu_nome","usuarios","usu_cod=".$colab[$rs_eve->fld("imp_depto")]["user"]))."</td>
							<td>
							...
							<td>
								...
								<span title='".$menv."' data-toggle='tooltip'>
								<input 
									class='chk_imp' ".$hide."
									type='checkbox' 
									value=1
...
if($acao == "exclui_doc"){
	...
	if(!$rs_eve->Exclui("documentos", $whr)){
		...
		$result['status'] = "OK";
		$result['mensagem'] = "Documento {$doc_id} excluído!";
	}
	else{
		$result['status'] = "NOK";
		$result['mensagem'] = "Ocorreu um erro!";
...
if($acao == "cli_Cadarquivo"){
	...
	$dados['cliarq_arqId'] 		= addslashes($arq_titulo);
