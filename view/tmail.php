...
// Pegando e-mail dos clientes
$sql = "SELECT * FROM irpf_mailsender WHERE ims_enviado = 0";
$rs->FreeSql($sql);
if($rs->linhas == 0 ){
	...
	while($rs->GeraDados()){
		...
		$assunto = 'ARQUIVOS IRPF - ORG. CONT. TRIANGULO';

		$headers = "From: informatica@triangulocontabil.com.br\r\n";
		$headers .= "Reply-To: informatica@triangulocontabil.com.br\r\n";
		...
		$destinatarios = ($rs->fld("ims_dest")<>""?$rs->fld("ims_dest"):"nilza@triangulocontabil.com.br");
		$nomeDestinatario = "IRPF";
		...
		if(!$mail->Send()){
			...
			$rs1->Altera($dados,"irpf_mailsender","ims_id=".$codims);
