$(document).on("ready", function(){
			...
			$.post("../controller/TRIMaquinas.php",{
				...
				function(data){
					if(data.status=="OK"){
						$("#slc").load("vis_maquinas.php").fadeIn("slow");
						$("#cad_maq")[0].reset();
						location.href = "maquinas.php?token=" +$("#token").val()+"&cnpj="+$("#cnpj").val();
					} 
					else{
						$("#slc").load("vis_maquinas.php").fadeIn("slow");
					}
			...
			$.post("../controller/TRIMaquinas.php",{
				...
				function(data){
					if(data.status=="OK"){
						//$("#slc").load("vis_maquinas.php").fadeIn("slow");
						//$("#cad_maq")[0].reset();
						location.href = "novo_periferico.php?token=" +$("#token").val()+"&cnpj="+$("#cnpj").val()+"&maqid="+$("#per_assoc").val()
					} 
					else{
			...
			$.post("../controller/TRIMaquinas.php",{
				...
				function(data){
					if(data.status=="OK"){
						...
						//$("#cad_maq")[0].reset();
						location.href = "novo_periferico.php?token="+$("#token").val()+"&cnpj="+$("#cnpj").val()+"&maqid="+$("#per_assoc").val();
						console.log("novo_periferico.php?token="+$("#token").val()+"&cnpj="+$("#cnpj").val()+"&maqid="+$("#per_assoc").val());
					} 
					else{
	...
	\*-------------------------------------------------------------*/
		$(document.body).on("click","#bt_search",function(){
			...
			$("#vis_maquinas").load("vis_maquinas.php?ip="+ip+"&user="+user+"&usuario="+usua+"&token="+token,
				function(){
             		$("#bt_search").html("<i class='fa fa-search'></i>");    
           		}
           	);
	...
	\*-------------------------------------------------------------*/
		$(document.body).on("click","#bt_searchp",function(){
			...
			$("#vis_perifericos").load("vis_perifericos.php?ip="+ip+"&tipo="+per+"&token="+token,
				function(){
             		$("#bt_searchp").html("<i class='fa fa-search'></i>");    
           		}
			);
