...
<?php
	...
	$sql .= " ORDER BY per_ativo DESC, per_tipo ASC, per_id ASC, c.maq_ip ASC";
	$rs->FreeSql($sql);
	//echo $rs->sql;
	if($rs->linhas==0):
	echo "<tr><td colspan=9> Nenhum periférico</td></tr>";
		...
		while($rs->GeraDados()){ ?>
			...
			<tr>
				...
				<td><?=$rs->fld("nmaq");?></td>
				<td>
					<a id="bt_novoper" class="btn btn-xs btn-info" data-toggle='tooltip' data-placement='bottom' title='Gerenciar Periféricos' href="novo_periferico.php?token=<?=$_SESSION['token'];?>&maqid=<?=$rs->fld("maq_id");?>&perid=<?=$rs->fld("per_id");?>"><i class="fa fa-keyboard-o"></i></a>
