				...
				<div class="box box-primary">
					<form id="cad_trib" role="form">
						...
						if(isset($_GET['tr_id']) && $_GET['tr_id'] <> "" ){
							$whr = "tr_id = ".$_GET['tr_id'];
							$rs2->Seleciona("*","tributos", $whr);
							$rs2->GeraDados();
							$lin = $rs2->linhas;
						}
						?>
							...
							<div class="row">
								<div class="form-group col-md-3">
									...
									<select id="tr_titulo" name="tr_titulo" class="form-control select2" style="width:100%">
										...
										<?php
											$sql = "SELECT * FROM tipos_impostos WHERE imp_tipo='T'";
											$rs->FreeSql($sql);
											while($rs->GeraDados()):	
											?>
												<option <?=($rs2->fld("tr_titulo") == $rs->fld("imp_id")?"SELECTED":"");?> value="<?=$rs->fld("imp_id");?>"><?=$rs->fld("imp_nome");?></option>
											<?php
											endwhile;
										?>
									</select>	
									...
									<input type="hidden" name="tr_id" id="tr_id" value="<?=$rs2->fld("tr_id");?>">
								</div>
