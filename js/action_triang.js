$(document).on("ready", function(){
	...
	/*--------------|ATRIBUIR EMPRESAS AOS USUARIOS|---------------*\
	| Author: 	Cleber Marrara Prado 								|
	| Version: 	1.0 												|
	| Email: 	cleber.marrara.prado@gmail.com 						|
	| Date: 														|
	| Alteração - Cleber Marrara Prado - 20/08/2018					|
	| Botao para atribuir empresas de acesso ao usuário 			|
	\*-------------------------------------------------------------*/
		$(document.body).on("click", "#bt_atr_empresas", function(){
			var checkeditems = "";
			var table = $('#user_empresas').DataTable();
			var data = table.$('input[type="checkbox"]').serializeArray();
			$.each(data, function(k, v){
				checkeditems +=  v['value']+",";
			});
			checkeditems = checkeditems.substring(0,checkeditems.length-1);
			$.post("../controller/TRIEmpresas.php",
            {
                acao     : "atribuir_empresas",
                empresas : checkeditems,
                user     : $("#user_colab").val()
            },
            function(data){
                if(data.status=="OK"){
                        $("<div></div>").addClass("alert alert-success alert-dismissable").html('<i class="fa fa-flag-checkered"></i> '+data.mensagem+' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_emp");
                        $("#fisPesq").trigger("click");
                        
                    } else{
                        $("<div></div>").addClass("alert alert-danger alert-dismissable").html('<i class="fa fa-times"></i> Ocorreu um erro! ('+data.mensagem+')<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').appendTo("#consulta_emp");
                    }
                    console.log(data.sql);//TO DO mensagem OK
            },
            "json"
	...
	\*-------------------------------------------------------------*/
		$(document.body).on("click", "#bt_doc_ent", function(){
		...
		$(document.body).on("click","#bt_horas", function(){
			...
			$("#cad_hora").validate({
	            ...
	            rules: {
	                ...
	                ch_hora_entrada : {required: true, minlength:8},
	                ch_hora_saida : {required: true, minlength:8}
	            },
	            messages: {
	                ch_colab 		: {required: "Escolha um colaborador!"},
	                ch_data 		: {required: "Data da sa&iacute;da obrigat&oacute;ria!"},
	                ch_hora_entrada	: {required: "Hora da entrada obrigat&oacute;ria!", minlength:"Formato inv&acute;lido!"},
	                ch_hora_saida 	: {required: "Hora da sa&iacute;da obrigat&oacute;ria!", minlength:"Formato inv&acute;lido!"}
	            },
	            highlight: function(element) {
	        		$(element).closest('.form-group').addClass('has-error');
	    		},
	        ...
	        if($("#cad_hora").valid()==true){
				...
				$.post("../controller/TRIEmpresas.php",{
					...
					ch_data			: $("#ch_data").val(),
					ch_hora_entrada	: $("#ch_hora_entrada").val(),
					ch_hora_saida	: $("#ch_hora_saida").val(),
					ch_hora_tipo	: $("#ch_pad").val(),
					},
					function(data){
						if(data.status=="OK"){
							//altera o status tal qual url
	        ...
	        if($("#cad_servicos").valid()==true){
				...
				$.post("../controller/TRIEmpresas.php",{
					...
					function(data){
						if(data.status=="OK"){
							//altera o status tal qual url
							$("#vis_serv").load("vis_servs.php").fadeIn("slow");
	...
	| Date: 	16/01/2018											|
	\*-------------------------------------------------------------*/
		$("#bt_ver_cpcs").on("click", function(){
			var emp 	= $("#cpc_emp").val();
			var din		= $("#cpc_di").val();
			var dif 	= $("#cpc_df").val();
			$("#cpcviews").load('cpc_rel.php?emp='+emp+'&di='+din+'&df='+dif);
			/*
			$("#bt_printsaidas").attr({
				'href':'rel_saidas_print.php?emp='+emp+'&di='+din+'&df='+dif+'&stat='+sts
			});
	...
	| Date: 	29.01.2018											|
	\*-------------------------------------------------------------*/
		$("#lismetas_emp").on("change", function(){
			//alert("Oi");
			$('#lismetas_obrig option[value!=0]').remove();
			$('#lismetas_obrig').val(0);

			$.post("../controller/TRIEmpresas.php",
				{
					acao : "combo_metas",
					mcod : $("#lismetas_emp").val(),
					mdep : $("#lismetas_dep").val()
				},
				function(data){

					$("#lismetas_obrig").append(data);
					console.log(data);
				},
				"html"
			);
